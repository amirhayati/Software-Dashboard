...
// Helper function to convert Gregorian date to Jalali (Persian/Shamsi)
// Accurate conversion using day-of-year method
function gregorianToJalali(date: Date): string {
	let gy = date.getFullYear();
	const gm = date.getMonth() + 1;
	const gd = date.getDate();

	// Arrays for days in each month
	...
	// Calculate day count
	const leap = Math.floor(gy / 4) - Math.floor(gy / 100) + Math.floor(gy / 400);
	const dayCount = 365 * gy + leap + dayOfYear;

	// Convert to Jalali
	jy += 33 * Math.floor(dayCount / 12053);
	const remainder1 = dayCount % 12053;

	jy += 4 * Math.floor(remainder1 / 1461);
	const remainder2 = remainder1 % 1461;

	if (remainder2 >= 366) {
		jy += Math.floor((remainder2 - 1) / 365);
		dayOfYear = (remainder2 - 1) % 365;
	} else {
		dayOfYear = remainder2;
	}

	// Calculate month and day
	let jm, jd;
	if (dayOfYear < 186) {
		jm = 1 + Math.floor(dayOfYear / 31);
		jd = 1 + (dayOfYear % 31);
	} else {
		jm = 7 + Math.floor((dayOfYear - 186) / 30);
...
const generateTickets = (): Ticket[] => {
	...
	for (let i = 1; i <= 375; i++) {
		// Distribute tickets evenly across the year (from today backwards to 1 year ago)
		const daysOffset = Math.floor(((375 - i) / 375) * 365);
		const ticketDate = new Date(today.getTime() - daysOffset * 24 * 60 * 60 * 1000);
		const jalaliDate = gregorianToJalali(ticketDate);

		// Only 2 tickets have no comments (TK-001 and TK-375)
		...
		<div className="space-y-8">
			...
			<div className="flex flex-col gap-2">
				...
				<p className="text-muted-foreground text-lg">
					List of 375 software and system-related issues and problems reported from the past year
				</p>
			</div>
